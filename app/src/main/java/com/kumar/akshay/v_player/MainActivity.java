...
public class MainActivity extends AppCompatActivity implements CustomLoader.onQueryUpdated,
    ...
    private int hasPermissionRead;
    private int hasPermissionWrite;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //checking Permission
        hasPermissionRead = ContextCompat.checkSelfPermission(this,READ_EXTERNAL_STORAGE);
        hasPermissionWrite = ContextCompat.checkSelfPermission(this, WRITE_EXTERNAL_STORAGE);

        //if Read and write permission not granted call requestPermission()
        if (hasPermissionRead != PERMISSION_GRANTED || hasPermissionWrite != PERMISSION_GRANTED) {
            // Request Permission
            ActivityCompat.requestPermissions(this, new String[]{READ_EXTERNAL_STORAGE,WRITE_EXTERNAL_STORAGE},READ_STORAGE_REQUEST_CODE);
    ...
    protected void onResume() {
        ...
        if (hasPermissionRead == PERMISSION_GRANTED) {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull final String[] permissions, @NonNull int[] grantResults) {
        ...
        Log.d(TAG, "onRequestPermissionsResult: requestCode = "+requestCode+" permission = "
                +Arrays.toString(permissions)+"grantResults = "+Arrays.toString(grantResults));

        // using switch maybe in the feature more permission is needed
        switch (requestCode) {
                ...
                if (grantResults.length > 0 && grantResults[0] != PERMISSION_GRANTED) {
                    ...
                    Snackbar.make(view, R.string.app_permission_Message, Snackbar.LENGTH_INDEFINITE).setAction("Grant Permission", new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            // This Condition used when user pressed button on Snackbar
                            // the shouldShowRequestPermissionRationale() method will return true if user did not cheked "do not ask again" box
                            if (ActivityCompat.shouldShowRequestPermissionRationale(MainActivity.this, permissions[0])) {

                                //if user did not checked the "do not ask again" box
                                ActivityCompat.requestPermissions(MainActivity.this, new String[]{READ_EXTERNAL_STORAGE,WRITE_EXTERNAL_STORAGE}, READ_STORAGE_REQUEST_CODE);
                            } else {
                                //if user checked the do not ask again box take him to settings
                                //creating intent
                    ...
                    }).show(); // after inserting all the component show the snackbar
